...
public final class EditorMenuService {
    ...
    public EditorMenuService(MiraCore core, DefinitionService definitions, CrateLocationService locations, PreviewService previews) {
        this.core = core;
    ...
    public void openMain(Player player) {
        ...
        inventory.setItem(11, GuiItems.item(Material.CHEST, core.messages().parse("&dCrates"), List.of(
                core.messages().parse("&7Definitions: &f" + definitions.crates().size()),
                core.messages().parse("&7Click to browse and preview"))));
        inventory.setItem(13, GuiItems.item(Material.TRIPWIRE_HOOK, core.messages().parse("&dKeys"), List.of(
                core.messages().parse("&7Definitions: &f" + definitions.keys().size()))));
        inventory.setItem(15, GuiItems.item(Material.NETHER_STAR, core.messages().parse("&dRarities"), List.of(
                core.messages().parse("&7Definitions: &f" + definitions.rarities().size()))));
        inventory.setItem(22, GuiItems.item(Material.BOOK, core.messages().parse("&fCommand Editor"), List.of(
                core.messages().parse("&7Use &f/mcrates help &7for create/edit commands."),
                core.messages().parse("&7All changes save immediately."))));
    ...
    public void handleClick(Player player, MiraInventoryHolder holder, int rawSlot) {
        switch (holder.type()) {
            case MAIN -> {
                if (rawSlot == 11) openCrates(player, 0);
                else if (rawSlot == 13) openKeys(player, 0);
                else if (rawSlot == 15) openRarities(player, 0);
            }
            case CRATES -> handleCrateList(player, holder, rawSlot);
            case KEYS -> handlePagedNavigation(player, holder, rawSlot, () -> openKeys(player, holder.page() - 1), () -> openKeys(player, holder.page() + 1));
            case RARITIES -> handlePagedNavigation(player, holder, rawSlot, () -> openRarities(player, holder.page() - 1), () -> openRarities(player, holder.page() + 1));
            default -> { }
        }
    }
    ...
    private void openCrates(Player player, int requestedPage) {
        ...
        for (int slot = 0; slot < PAGE_SIZE && start + slot < values.size(); slot++) {
            ...
            inventory.setItem(slot, GuiItems.item(crate.icon(), core.messages().parse(crate.displayName()), List.of(
                    ...
                    core.messages().parse("&7Locations: &f" + locations.countForCrate(crate.id())),
                    core.messages().parse("&eClick to preview"))));
        }
        nav(inventory);
        player.openInventory(inventory);
    }
    ...
    private void handleCrateList(Player player, MiraInventoryHolder holder, int rawSlot) {
        ...
        if (index < values.size()) previews.open(player, values.get(index).id(), 0);
    }
